          ...
          <v-form ref="form" v-model="isFormValid" class="igf-registration-form" autocomplete="on" @submit.prevent="submitRegistration">
            <div class="igf-field-grid">
              <template v-for="field in formFields" :key="field.key">
                <label v-if="field.key === 'cause_id'" class="igf-field--wide igf-native-field" for="volunteer-cause"><span>{{ registrationFieldLabel(field.key) }}</span><select id="volunteer-cause" v-model="registration.cause_id" :aria-label="registrationFieldLabel(field.key)" :disabled="causes.length === 0" required><option :value="null" disabled>{{ settings.cause_placeholder }}</option><option v-for="cause in causes" :key="cause.id" :value="cause.id">{{ cause.name }}</option></select></label>
                <v-textarea
                  v-else-if="field.key === 'address'"
                  :model-value="registrationFieldValue(field.key)"
                  class="igf-field--wide"
                  :label="registrationFieldLabel(field.key)"
                  autocomplete="street-address"
                  variant="outlined"
                  rows="2"
                  auto-grow
                  hide-details="auto"
                  :rules="registrationFieldRules(field)"
                  :required="field.required"
                  @update:model-value="setRegistrationFieldValue(field.key, $event)"
                />
                <v-text-field
                  v-else
                  :model-value="registrationFieldValue(field.key)"
                  :label="registrationFieldLabel(field.key)"
                  :autocomplete="registrationFieldAutocomplete(field.key)"
...
const defaultFormFields = [
  ...
  { key: 'institution', enabled: true, required: true },
  { key: 'email', enabled: true, required: true },
  { key: 'phone', enabled: true, required: true },
  { key: 'address', enabled: true, required: true },
  { key: 'cause_id', enabled: true, required: true },
];
const registrationKeys = new Set(defaultFormFields.map(field => field.key));
const formFields = computed(() => {
  const managed = Array.isArray(settings.value.form_fields) ? settings.value.form_fields : defaultFormFields;
  return managed.filter(field => field && registrationKeys.has(field.key) && field.enabled !== false);
});
...
function registrationFieldValue(key) { return registration.value[key]; }
function setRegistrationFieldValue(key, value) { registration.value[key] = value; }
function registrationFieldLabel(key) {
  return {
    name: settings.value.name_field_label,
    institution: settings.value.institution_field_label,
    email: settings.value.email_field_label,
    phone: settings.value.phone_field_label,
    address: settings.value.address_field_label,
    cause_id: settings.value.cause_field_label,
  }[key] || key;
}
function registrationFieldAutocomplete(key) { return { name: 'name', institution: 'organization', email: 'email', phone: 'tel' }[key]; }
function registrationFieldRules(field) {
  if (field.key === 'email') return emailRules.value;
  return field.required ? [required(registrationFieldLabel(field.key))] : [];
}
