...
@section('content')
@php
    $canCreate = app(\App\Http\Middleware\Permission::class)->allows(auth('admin')->user(), 'division.store');
            ...
            <div id="new_division">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">{{ $Lang->Common->New }} {{ $Lang->DivisionTitle }}</strong>
                    </div>
                    <div class="card-body">
                        <div id="pay-invoice">
                            <div class="card-body">
        ...
        <div class="{{ $canCreate ? 'col-lg-7' : 'col-lg-12' }} col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-6">
                            <strong class="card-title">{{ $Lang->DivisionTitle }} {{ $Lang->Common->List }}</strong>
                        </div>
                        <div class="col-md-6">
                ...
                <div class="table-stats ov-h">
                    ...
                    <div class="pagination justify-content-end">
                        {{ $divisions->appends(['search' => $search])->links('vendor.pagination.bootstrap-4') }}
                    </div>
                </div>
            </div>
        </div>
...
@section('custom-js')
<script>
    itemDelete({tableId: "division_table",method: "DELETE"});
    itemStatus({tableId: "division_table",method: "PUT"});
    ...
    var is_edit = "{{old('id')}}";
    if (is_edit) {
        $('#new_division .form-group .help-block').hide();
        $("#new_division input:not([type=hidden])").val("");
