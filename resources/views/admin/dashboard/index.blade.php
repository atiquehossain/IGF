...
@section('content')
@php($ui = static fn (string $key, array $replace = []): string => \App\Support\AdminUi::text($key, $replace))
<style>
    ...
    .dash-body{display:grid;grid-template-columns:minmax(0,2fr) minmax(300px,1fr);gap:22px}.dash-body--single{grid-template-columns:1fr}.dash-body--single .dash-side{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr))}.dash-chart{display:flex;flex-direction:column;min-height:506px;padding:23px 24px 22px}.dash-card-head{display:flex;align-items:center;justify-content:space-between;gap:15px;margin-bottom:24px}.dash-card-head h2{margin:0;font:600 19px/1.3 'Literata',serif}.dash-period{padding:7px 11px;border:1px solid var(--dash-line);border-radius:8px;background:#f8f9fa;color:#5e5d66;font-size:11px;font-weight:700}
...
<main class="igf-dashboard">
    <header class="dash-heading">
        <h1>{{ $ui('dashboard.overview') }}</h1>
        <p>{{ $ui('dashboard.intro') }}</p>
    </header>

    @if($quickActions->isNotEmpty() || $canReviewAnyEnquiry)
        <section class="dash-quick" aria-labelledby="quick-actions-title">
            <div class="dash-quick-head">
                <h2 id="quick-actions-title">{{ $quickActions->isNotEmpty() ? $ui('dashboard.common_tasks') : $ui('dashboard.needs_review') }}</h2>
                @if($canReviewAnyEnquiry)
                    @if($newEnquiries > 0)
                        <nav class="dash-enquiries" aria-label="{{ $ui('dashboard.new_enquiries') }}">
                            <span>{{ $ui($newEnquiries === 1 ? 'dashboard.one_item_review' : 'dashboard.many_items_review', ['count' => number_format($newEnquiries)]) }}</span>
                            @foreach($enquiryActions as $enquiry)
                                <a href="{{ route($enquiry['route']) }}"><strong>{{ number_format($enquiry['count']) }}</strong>{{ $enquiry['label'] }}</a>
                            @endforeach
                        </nav>
                    @else
                        <span class="dash-enquiries--clear"><i class="fa fa-check-circle-o" aria-hidden="true"></i> {{ $ui('dashboard.no_new_enquiries') }}</span>
                    @endif
                @endif
            </div>
    ...
    @if($siteHealth->isNotEmpty())
        <section class="dash-health" aria-label="{{ $ui('dashboard.setup_alerts') }}">
    ...
    @if($canReviewDonations || $canReviewVolunteers)
        <section class="dash-metrics" aria-label="{{ $ui('dashboard.foundation_metrics') }}">
            @if($canReviewDonations)
                <article class="dash-card dash-metric">
                    <div class="dash-metric-top"><span class="dash-label">{{ $ui('dashboard.donations_today') }}</span><span class="dash-icon"><i class="fa fa-heart-o" aria-hidden="true"></i></span></div>
                    <div class="dash-value">৳{{ number_format($metrics['donations_today']) }}</div>
                    <div class="dash-delta"><i class="fa {{ $metrics['donation_change'] >= 0 ? 'fa-line-chart' : 'fa-level-down' }}" aria-hidden="true"></i><span>{{ $metrics['donation_change'] >= 0 ? '+' : '' }}{{ $metrics['donation_change'] }}% {{ $ui('dashboard.from_yesterday') }}</span></div>
                </article>
                <article class="dash-card dash-metric">
                    <div class="dash-metric-top"><span class="dash-label">{{ $ui('dashboard.successful_gifts') }}</span><span class="dash-icon"><i class="fa fa-refresh" aria-hidden="true"></i></span></div>
                    <div class="dash-value">{{ number_format($metrics['successful_gifts']) }}</div>
                    <div class="dash-delta"><i class="fa fa-line-chart" aria-hidden="true"></i><span>+{{ number_format($metrics['successful_this_month']) }} {{ $ui('dashboard.this_month') }}</span></div>
                </article>
                <article class="dash-card dash-metric">
                    <div class="dash-metric-top"><span class="dash-label">{{ $ui('dashboard.pending_gateways') }}</span><span class="dash-icon dash-icon--danger"><i class="fa fa-exclamation-circle" aria-hidden="true"></i></span></div>
                    <div class="dash-value">{{ number_format($metrics['pending_gateways']) }}</div>
                    <div class="dash-delta {{ $metrics['pending_gateways'] ? 'dash-delta--danger' : 'dash-delta--neutral' }}"><i class="fa {{ $metrics['pending_gateways'] ? 'fa-warning' : 'fa-check-circle-o' }}" aria-hidden="true"></i><span>{{ $metrics['pending_gateways'] ? $ui('dashboard.requires_attention') : $ui('dashboard.gateways_clear') }}</span></div>
                </article>
            @endif
            @if($canReviewVolunteers)
                <article class="dash-card dash-metric">
                    <div class="dash-metric-top"><span class="dash-label">{{ $ui('dashboard.volunteers') }}</span><span class="dash-icon"><i class="fa fa-users" aria-hidden="true"></i></span></div>
                    <div class="dash-value">{{ number_format($metrics['volunteers']) }}</div>
                    <div class="dash-delta dash-delta--neutral"><i class="fa fa-check-circle-o" aria-hidden="true"></i><span>{{ $ui('dashboard.active_approved') }}</span></div>
                </article>
            @endif
        </section>
    @endif

    @if($canReviewDonations || $hasSideDashboardCards)
    <section class="dash-body {{ !$canReviewDonations || !$hasSideDashboardCards ? 'dash-body--single' : '' }}">
        @if($canReviewDonations)
        <article class="dash-card dash-chart">
            <div class="dash-card-head"><h2>{{ $ui('dashboard.revenue_trends') }}</h2><span class="dash-period">{{ $ui('dashboard.last_seven_months') }}</span></div>
            <div class="dash-chart-plot" role="img" aria-label="{{ $ui('dashboard.donation_revenue_chart') }}">
        ...
        @if($hasSideDashboardCards)
        <div class="dash-side">
            @if($canReviewLocalization)
            <article class="dash-card dash-localization">
                <h2>{{ $ui('dashboard.localization_tracker') }}</h2>
                <div class="locale-list">
                    @foreach ($localization as $locale)
                        <div class="locale-item">
                            <div class="locale-meta"><strong>{{ $locale['label'] }} ({{ strtoupper($locale['locale']) }})</strong><span>{{ $locale['percent'] }}% {{ $ui('dashboard.complete') }}</span></div>
                            <div class="locale-track" role="progressbar" aria-label="{{ $ui('dashboard.content_completion', ['language' => $locale['label']]) }}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="{{ $locale['percent'] }}"><div class="locale-fill" style="width:{{ $locale['percent'] }}%"></div></div>
                        </div>
                    @endforeach
                </div>
                @if($canReviewTranslations)<a class="dash-secondary-button" href="{{ route('translations.index') }}">{{ $ui('dashboard.review_missing') }}</a>@endif
            </article>
            @endif

            @if($canReviewActivity)
            <article class="dash-card dash-activity">
                <div class="dash-card-head dash-card-head--activity"><h2>{{ $ui('dashboard.recent_activity') }}</h2>@if($canReviewPages)<a href="{{ route('page.index') }}">{{ $ui('dashboard.view_all') }}</a>@endif</div>
                @if ($recentActivity->isEmpty())
                    <div class="activity-empty">{{ $ui('dashboard.activity_empty') }}</div>
                @else
    ...
    @unless($hasDashboardInsights)
        <section class="dash-health" aria-label="{{ $ui('dashboard.needs_review') }}">
            <article class="dash-health-item">
                <span><strong>{{ $ui('dashboard.needs_review') }}</strong><small>{{ $ui('dashboard.ask_teammate') }}</small></span>
            </article>
        </section>
    @endunless
</main>
