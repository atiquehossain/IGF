      ...
      <section class="igf-auth__panel" aria-labelledby="login-title">
        <div class="igf-auth__card">
          ...
          <form @submit.prevent="login">
            <label for="login-phone">{{ content.phone_label }}<input id="login-phone" v-model="form.phone_no" type="tel" inputmode="numeric" autocomplete="tel" maxlength="11" :placeholder="content.phone_placeholder" required :aria-invalid="form.errors.phone_no ? 'true' : undefined" :aria-describedby="form.errors.phone_no ? 'login-phone-error' : undefined"></label><p v-if="form.errors.phone_no" id="login-phone-error" class="igf-error" role="alert">{{ form.errors.phone_no }}</p>
            <label for="login-password">{{ content.password_label }}<span class="igf-password"><input id="login-password" v-model="form.password" :type="showPassword ? 'text' : 'password'" autocomplete="current-password" required :aria-invalid="form.errors.password ? 'true' : undefined" :aria-describedby="form.errors.password ? 'login-password-error' : undefined"><button type="button" :aria-label="showPassword ? content.hide_password_label : content.show_password_label" @click="showPassword = !showPassword"><i :class="showPassword ? 'fa-regular fa-eye-slash' : 'fa-regular fa-eye'" aria-hidden="true" /></button></span></label><p v-if="form.errors.password" id="login-password-error" class="igf-error" role="alert">{{ form.errors.password }}</p>
...
@media(max-width:900px){.igf-auth{grid-template-columns:1fr}.igf-auth__story{min-height:390px}.igf-auth__panel{padding:55px 22px}.igf-auth__back{margin-bottom:40px}}@media(max-width:450px){.igf-auth__story{padding:35px 24px}.igf-auth__story h1{font-size:42px}}
</style>
