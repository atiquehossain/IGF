...
<main class="seo2">
    ...
    <section class="seo2-card" style="margin-bottom:18px">
        <div class="seo2-card__body"><form class="seo2-filter" method="GET" action="{{ route('seo.bulk.index') }}">
            ...
            <label class="seo2-field"><span>Content type</span><select name="type">@foreach(['all'=>'All content','page'=>'Pages','category'=>'Categories','event'=>'Events & publications','annual_report'=>'Annual reports','project'=>'Projects','donation_cause'=>'Donation causes','job'=>'Jobs','workshop'=>'Workshops','route'=>'Website features'] as $key=>$label)<option value="{{ $key }}" @selected($filters['type'] === $key)>{{ $label }}</option>@endforeach</select></label>
