    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex flex-wrap align-items-center justify-content-between" style="gap:12px">
                ...
                @if($search !== '')<form action="{{ route('contact-message.search.clear') }}" method="post" class="d-inline">@csrf<button type="submit" class="btn btn-light btn-sm mb-2">Clear private search</button></form>@endif
                <form action="{{ route('contact-message.index') }}" method="get" class="form-inline" aria-label="Filter contact enquiries by status">
                    ...
                    <button type="submit" class="btn btn-info btn-sm"><i class="fa fa-filter" aria-hidden="true"></i> Filter</button>
                </form>
                </div>
            </div>
        </div>
