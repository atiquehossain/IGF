    ...
    <form method="POST" action="{{ route('seo.bulk.update') }}" data-seo-bulk-form>
        ...
        <div class="seo2-bulk-wrap">
            <table class="seo2-bulk">
                <thead><tr><th><label class="seo2-bulk__select"><input type="checkbox" data-bulk-select-all @disabled(!$editableTargetCount || !$canEditMetadata)> Select all editable rows</label>Content</th><th>Source</th><th>Search title</th><th>Search description</th><th>Social image URL</th><th>Visibility</th><th>Schema</th></tr></thead>
                <tbody>
                @forelse($targets as $index => $target)
                    ...
                    <tr data-bulk-row class="{{ $rowEditable ? 'is-unselected' : '' }}">
                        ...
                        <td class="seo2-bulk__image"><input type="url" name="items[{{ $index }}][image]" value="{{ $target['stored']['image'] }}" placeholder="https://…" aria-label="Social image URL for {{ $controlLabel }}" @disabled(!$rowEditable)>@if($canViewMedia && $rowEditable)<a class="seo2-help" href="{{ route('media.index', ['type'=>'image']) }}" target="_blank" rel="noopener">Open Media Library</a>@endif</td>
