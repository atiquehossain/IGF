...
<template>
  <Layout>
    <div v-if="listing" class="igf-opportunity-detail">
      <aside v-if="preview.enabled" class="igf-opportunity-preview" role="status">
        <div class="igf-shell">
          <div><strong>{{ preview.label }}</strong><span>{{ preview.message }}</span></div>
          <a :href="preview.editor_url">{{ preview.editor_label }}</a>
        </div>
      </aside>
      <header class="igf-opportunity-detail__hero">
        <div class="igf-shell">
          <a :href="route('frontend.jobs.index')"><span aria-hidden="true">←</span> {{ copy.back_label }}</a>
          <div class="igf-opportunity-detail__status-row">
            <p>{{ listing.department || copy.eyebrow }}</p>
            <span v-if="listing.status_label">{{ listing.status_label }}</span>
          </div>
          <h1>{{ listing.title }}</h1>
      ...
      <div class="igf-opportunity-detail__main">
        <article class="igf-opportunity-detail__article">
          ...
          <section v-if="listing.responsibilities" aria-labelledby="job-responsibilities-title">
            <h2 id="job-responsibilities-title">{{ copy.responsibilities_title }}</h2>
            <div v-html="listing.responsibilities" />
          </section>
          <section v-if="listing.requirements" aria-labelledby="job-requirements-title">
            <h2 id="job-requirements-title">{{ copy.requirements_title }}</h2>
            <div v-html="listing.requirements" />
          </section>
        </article>

        <aside
          class="igf-opportunity-detail__form-panel"
          :aria-labelledby="submissionReference ? undefined : (isOpen ? 'job-form-title' : 'job-closed-title')"
        >
          <SubmissionReference
            v-if="submissionReference"
          ...
          <template v-else-if="isOpen">
            <header class="igf-opportunity-detail__form-head">
              <p>{{ copy.form_eyebrow }}</p>
              <h2 id="job-form-title">{{ copy.form_title }}</h2>
              <span>{{ copy.form_introduction }}</span>
            </header>
            <SchemaForm
              ref="schemaForm"
              v-model="answers"
              ...
              :submit-label="copy.submit_label"
              :privacy-message="copy.privacy_message"
              :form-token="data.form?.token || ''"
              :honeypot-name="data.form?.honeypot_name || 'company_website'"
              require-cv
          ...
          <section v-else class="igf-opportunity-detail__closed" role="status">
            ...
            <h2 id="job-closed-title">{{ closedTitle }}</h2>
            <p>{{ closedMessage }}</p>
          </section>
        </aside>
      </div>
...
const copy = computed(() => ({
  ...
  opens_label: 'Applications open',
  deadline_label: 'Application deadline',
  requirements_title: 'Requirements',
  responsibilities_title: 'Responsibilities',
  form_eyebrow: 'Application form',
  form_title: 'Apply for this position',
  ...
  cv_label: 'CV (PDF)',
  cv_help: 'Upload one PDF file. Maximum size: 5 MB.',
  submit_label: 'Submit application',
  privacy_message: 'Your application and CV are private and available only to authorized recruitment staff.',
  closed_title: 'Applications closed',
  closed_message: 'This position is no longer accepting applications.',
  upcoming_title: 'Applications are not open yet',
  upcoming_message: 'Please return when the application period begins.',
  ...(page.props.copy || {}),
  ...(data.value.copy || {}),
}))
...
const fields = computed(() => withFixedFields(normalizeFormFields(data.value.form), [
  ...
  { uuid: 'fixed-cv', key: 'cv', type: 'file', label: copy.value.cv_label, help: copy.value.cv_help, required: true, validation: { accept: ['.pdf', 'application/pdf'], max_file_size_mb: 5 } },
]))
const metadata = computed(() => [
  [copy.value.location_label, listing.value?.location],
  [copy.value.employment_type_label, listing.value?.employment_type_label || listing.value?.employment_type],
  [copy.value.work_arrangement_label, listing.value?.work_arrangement_label],
  [copy.value.vacancies_label, listing.value?.vacancies_label || listing.value?.vacancies],
  [copy.value.opens_label, listing.value?.application_opens_label],
  [copy.value.deadline_label, listing.value?.application_deadline_label || listing.value?.deadline_label],
...
const isUpcoming = computed(() => ['upcoming', 'scheduled', 'not_open'].includes(String(listing.value?.application_state || listing.value?.state || '').toLocaleLowerCase()))
...
function listingAcceptsSubmissions(value) {
  ...
  for (const key of ['is_open', 'can_apply', 'accepting_applications']) {
  ...
  return !['closed', 'expired', 'upcoming', 'scheduled', 'not_open', 'draft', 'paused', 'cancelled'].includes(
    String(value.application_state || value.state || '').toLocaleLowerCase(),
  )
}

function submitApplication(currentAnswers, security = {}) {
  ...
  const actionUrl = listingActionUrl(listing.value.public_url, 'apply')
    || route('frontend.jobs.apply', listing.value.slug)
