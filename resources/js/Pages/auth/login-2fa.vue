  ...
  <GuestLayout>
    <div class="igf-auth">
      ...
      <section class="igf-auth__panel" aria-labelledby="secure-login-title"><div class="igf-auth__card"><a class="igf-auth__back" :href="route('frontend.home')"><span aria-hidden="true">&larr;</span> {{ content.back_label }}</a><p class="igf-eyebrow">{{ content.two_factor_eyebrow }}</p><h2 id="secure-login-title">{{ content.two_factor_title }}</h2><p class="igf-auth__intro">{{ content.two_factor_introduction }}</p>
        <form @submit.prevent="login"><label for="secure-email">{{ content.two_factor_email_label }}<input id="secure-email" v-model="form.email" type="email" autocomplete="username" required></label><p v-if="form.errors.email" class="igf-error" role="alert">{{ form.errors.email }}</p><label for="secure-password">{{ content.two_factor_password_label }}<span class="igf-password"><input id="secure-password" v-model="form.password" :type="showPassword ? 'text' : 'password'" autocomplete="current-password" required><button type="button" :aria-label="showPassword ? content.hide_password_label : content.show_password_label" @click="showPassword = !showPassword"><i :class="showPassword ? 'fa-regular fa-eye-slash' : 'fa-regular fa-eye'" aria-hidden="true" /></button></span></label><p v-if="form.errors.password" class="igf-error" role="alert">{{ form.errors.password }}</p><button class="igf-submit" type="submit" :disabled="form.processing">{{ form.processing ? content.two_factor_sending_label : content.two_factor_submit_label }} <i class="fa-solid fa-arrow-right" aria-hidden="true" /></button></form>
