...
<main class="igf-trash">
    <header class="igf-trash__head"><div><h1>Navigation trash</h1><p>@if($canRestoreMenu || $canPermanentlyDeleteMenu)Restore menu items or permanently remove them after their children are handled.@else Read-only access: you can review deleted navigation items, but your role cannot restore or permanently delete them.@endif</p></div>@if($canViewNavigation)<a class="btn igf-btn igf-btn-secondary" href="{{ route('page.menu.index') }}"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back to navigation</a>@endif</header>
    <form class="igf-search" method="GET"><label class="sr-only" for="navigation-trash-search">Search trashed navigation</label><input id="navigation-trash-search" type="search" name="search" value="{{ $search }}" placeholder="Search trashed navigation"><button class="igf-btn" type="submit">Search</button></form>
    <div class="igf-table-wrap table-responsive" role="region" aria-label="Scrollable trashed navigation records" tabindex="0"><table class="igf-table"><thead><tr><th>Name</th><th>Location</th><th>Deleted</th><th>Actions</th></tr></thead><tbody>
