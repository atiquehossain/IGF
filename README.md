...
Paginated Events & News, Projects, Gallery, and Annual Reports archives follow one canonical policy: a clean page 2 or later self-canonicalizes and receives a page-numbered title; search/filter, malformed-page, and unknown-query variants are `noindex,follow` and canonicalize to the clean archive; an out-of-range page returns `404`. Sitemap entries are generated only from safe, public, indexable content. The generated `/robots.txt` advertises the sitemaps and allows public crawling while keeping `/admin` disallowed; when environment-wide indexing is disabled, page-level robots metadata and the matching `X-Robots-Tag` response header apply `noindex,nofollow,noarchive` so crawlers can observe the directive.
...
The optional **Search Performance** center reads first-party Google Search Console clicks, impressions, click-through rates, queries, pages, and sitemap status, plus Google Analytics 4 organic landing-page sessions when a deployment owner configures the approved properties and read-only service account. It does not scrape results, track keyword positions, use AI, collect Core Web Vitals, or run Lighthouse/PageSpeed tests. Dashboard completion scores still describe configured content rather than live search or traffic performance. See `docs/NO_CODE_ADMIN_GUIDE.md` for the editor workflow.
