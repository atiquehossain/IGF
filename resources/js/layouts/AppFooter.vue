    ...
    <div class="site-footer__shell">
      ...
      <div class="footer-body" :class="{ 'has-legal-status': legalStatusColumn }">
        <div class="footer-brand">
          ...
          <p v-if="footer.about" class="footer-brand__about">{{ footer.about }}</p>
          <p v-if="footer.trustBadge" class="footer-brand__trust">
          ...
          >
            ...
            >
              <h2 class="footer-link-group__heading">
                <span class="footer-link-group__title">
                  <a v-if="column.href !== '#'" :href="column.href">{{ column.title }}</a>
                  <span v-else>{{ column.title }}</span>
                </span>
                <button
                  v-if="compactFooter && column.items.length"
              ...
              >
                <ManagedMenuTree :items="column.items" :depth="2" :max-depth="3" />
              </div>
            </section>
          </nav>
...
import { publicMenuHref } from '../Shared/utils/publicMenu';
...
<style scoped>
.site-footer { --footer-primary:var(--igf-primary,#ff7500); --footer-accent:var(--igf-accent,#9c4500); padding:clamp(42px,5vw,58px) clamp(20px,4vw,48px) 24px; background:var(--igf-footer-bg,#202124); color:var(--igf-footer-text,#d2d3d4); font-family:var(--igf-font-body,'Hanken Grotesk',Arial,sans-serif); }
.site-footer__shell { width:min(100%,var(--igf-footer-shell-width,1440px)); margin:0 auto; }
.footer-newsletter { display:grid; grid-template-columns:minmax(200px,.75fr) minmax(360px,1.25fr); align-items:center; gap:clamp(24px,4vw,52px); margin-bottom:32px; padding-bottom:26px; border-bottom:1px solid var(--igf-footer-border,rgba(255,255,255,.12)); }
.footer-newsletter h2 { margin:0 0 6px; color:var(--igf-footer-strong,#fff); font:650 21px/1.2 var(--igf-font-heading,'Literata',Georgia,serif); }
...
.footer-newsletter p { margin:0; color:var(--igf-footer-muted,#b9bbbd); font-size:13px; line-height:1.5; }
.footer-newsletter__row { display:grid; grid-template-columns:minmax(0,1fr) auto; gap:8px; }
.footer-newsletter__row input { min-width:0; min-height:44px; padding:10px 12px; border:1px solid var(--igf-footer-field-border,rgba(255,255,255,.24)); border-radius:var(--igf-radius-sm,8px); background:var(--igf-footer-field-bg,#fff); color:var(--igf-footer-field-text,#202124); }
.footer-newsletter__row button { min-height:44px; padding:10px 17px; border:0; border-radius:var(--igf-radius-sm,8px); background:var(--footer-primary); color:var(--igf-on-primary,#000); font-weight:850; cursor:pointer; }
.footer-newsletter__row button:disabled { cursor:wait; opacity:.65; }
.footer-newsletter__consent { display:flex; align-items:flex-start; gap:7px; margin-top:8px; color:var(--igf-footer-muted,#b9bbbd); font-size:10px; line-height:1.45; }
.footer-newsletter__consent input { flex:0 0 auto; margin-top:2px; accent-color:var(--footer-primary); }
.footer-newsletter__consent a { color:var(--igf-footer-link-hover,color-mix(in srgb,var(--footer-primary) 40%,#fff)); }
.footer-newsletter__message { margin:8px 0 0!important; padding:8px 10px; border:1px solid transparent; border-radius:var(--igf-radius-sm,7px); }
.footer-newsletter__message.is-success { border-color:var(--igf-footer-success-border,rgba(119,221,151,.45)); background:var(--igf-footer-success-bg,rgba(38,129,68,.2)); color:var(--igf-footer-success-text,#c9f7d7)!important; }
.footer-newsletter__message.is-error { border-color:var(--igf-footer-error-border,rgba(255,138,128,.55)); background:var(--igf-footer-error-bg,rgba(176,51,40,.22)); color:var(--igf-footer-error-text,#ffd8d4)!important; }
.footer-newsletter__row input[aria-invalid="true"] { border-color:#ff8a80; outline:3px solid rgba(255,138,128,.22); }
.footer-body { display:grid; grid-template-columns:var(--igf-footer-body-columns,minmax(220px,.9fr) minmax(0,2.1fr)); align-items:start; gap:clamp(28px,4vw,48px); }
.footer-body.has-legal-status { grid-template-columns:var(--igf-footer-body-legal-columns,minmax(250px,.85fr) minmax(0,3.15fr)); gap:clamp(36px,4vw,64px); }
.footer-content { min-width:0; }
.footer-content.has-legal-status { display:grid; grid-template-columns:var(--igf-footer-content-legal-columns,minmax(0,2.1fr) minmax(300px,1fr)); align-items:start; gap:clamp(30px,3vw,48px); }
.footer-brand { min-width:0; }
.footer-brand__name { display:inline-flex; align-items:center; color:var(--igf-footer-strong,#fff); text-decoration:none; }
...
.footer-brand__tagline { display:block; max-width:360px; margin-top:10px; color:var(--igf-footer-link-hover,color-mix(in srgb,var(--footer-primary) 48%,#fff)); font-size:12px; font-weight:800; line-height:1.4; }
.footer-brand__tagline span { display:block; }
.footer-brand__about { max-width:390px; margin:12px 0 0; color:var(--igf-footer-muted,#b9bbbd); font-size:12px; line-height:1.55; }
.footer-brand__trust { display:inline-flex; max-width:390px; align-items:center; gap:8px; margin:12px 0 0; padding:7px 10px; border:1px solid var(--igf-footer-border,rgba(255,255,255,.12)); border-radius:var(--igf-radius-sm,8px); background:var(--igf-footer-badge-bg,color-mix(in srgb,var(--footer-primary) 12%,transparent)); color:var(--igf-footer-badge-text,color-mix(in srgb,var(--footer-primary) 40%,#fff)); font-size:11px; font-weight:750; line-height:1.35; }
.footer-contact-block { max-width:390px; margin:16px 0; }
.footer-contact { display:grid; gap:8px; margin:0; font-style:normal; }
.footer-contact a,.footer-contact span { display:flex; min-width:0; align-items:flex-start; gap:8px; color:var(--igf-footer-text,#c9cbcc); font-size:12px; line-height:1.45; text-decoration:none; overflow-wrap:anywhere; }
...
.footer-contact__copy strong { color:var(--igf-footer-strong,#fff); font-weight:700; }
.footer-social { display:flex; flex-wrap:wrap; gap:8px; }
.footer-social a { display:grid; width:36px; height:36px; place-content:center; border-radius:50%; background:var(--igf-footer-social-bg,rgba(255,255,255,.08)); color:var(--igf-footer-text,#e0e1e2); text-decoration:none; }
.footer-social a:hover { background:var(--footer-primary); color:var(--igf-on-primary,#000); }
...
.footer-link-group__heading,.footer-legal-status h2 { margin:0 0 12px; color:var(--igf-footer-heading,color-mix(in srgb,var(--footer-primary) 62%,#fff)); font:800 12px/1.25 var(--igf-font-body,'Hanken Grotesk',Arial,sans-serif); letter-spacing:.07em; text-transform:uppercase; }
.footer-link-group__heading { display:flex; min-width:0; align-items:center; justify-content:space-between; gap:8px; }
.footer-link-group__title { min-width:0; }
.footer-link-group__title>a { color:inherit; font:inherit; letter-spacing:inherit; text-transform:inherit; }
.footer-link-group__links { min-width:0; }
.footer-navigation :deep(.managed-menu-tree) { display:grid; gap:9px; width:100%; margin:0; padding:0; list-style:none; }
.footer-navigation :deep(.managed-menu-tree[data-menu-depth="3"]) { gap:7px; margin-top:7px; padding-left:12px; border-left:1px solid var(--igf-footer-border,rgba(255,255,255,.16)); }
.footer-navigation :deep(.managed-menu-tree__item) { min-width:0; }
.footer-navigation :deep(.managed-menu-tree__link),.footer-navigation :deep(.managed-menu-tree__label) { color:var(--igf-footer-link,#c5c7c8); font-size:13px; line-height:1.4; text-decoration:none; overflow-wrap:anywhere; }
.footer-navigation :deep(.managed-menu-tree__label) { display:block; color:var(--igf-footer-strong,#fff); font-weight:750; }
.footer-navigation :deep(.managed-menu-tree[data-menu-depth="3"] .managed-menu-tree__link),.footer-navigation :deep(.managed-menu-tree[data-menu-depth="3"] .managed-menu-tree__label) { font-size:12px; }
.footer-links a { color:var(--igf-footer-link,#c5c7c8); font-size:13px; line-height:1.4; text-decoration:none; overflow-wrap:anywhere; }
.footer-links a:hover { color:var(--igf-footer-link-hover,color-mix(in srgb,var(--footer-primary) 48%,#fff)); }
.footer-link-group__toggle { display:grid; width:44px; min-width:44px; height:44px; place-content:center; padding:0; border:0; background:transparent; color:inherit; cursor:pointer; }
...
.footer-legal-status__logo,.footer-legal-status__badge { display:grid; width:34px; height:34px; place-items:center; border:1px solid var(--igf-footer-border,rgba(255,155,72,.36)); border-radius:var(--igf-radius-md,9px); background:var(--igf-footer-field-bg,#fff); }
.footer-legal-status__logo { object-fit:contain; padding:4px; }
.footer-legal-status__badge { background:var(--igf-footer-badge-bg,color-mix(in srgb,var(--footer-primary) 12%,transparent)); color:var(--igf-footer-badge-text,color-mix(in srgb,var(--footer-primary) 40%,#fff)); font-size:8px; font-weight:900; letter-spacing:.03em; text-align:center; }
.footer-legal-status__copy { display:grid; gap:2px; min-width:0; }
.footer-legal-status__copy strong { color:var(--igf-footer-strong,#e9eaeb); font-size:11px; font-weight:800; letter-spacing:.015em; line-height:1.3; text-transform:uppercase; overflow-wrap:anywhere; }
.footer-legal-status__copy small { color:var(--igf-footer-badge-text,color-mix(in srgb,var(--footer-primary) 40%,#fff)); font-size:12px; font-weight:700; font-variant-numeric:tabular-nums; line-height:1.3; overflow-wrap:anywhere; }
.footer-bottom { display:flex; align-items:center; justify-content:center; margin-top:28px; padding-top:19px; border-top:1px solid var(--igf-footer-border,rgba(255,255,255,.12)); color:var(--igf-footer-muted,#b5b7b9); font-size:11px; line-height:1.45; text-align:center; }
...
@media(max-width:1199px) {
  .footer-body.has-legal-status { grid-template-columns:var(--igf-footer-body-tablet-columns,minmax(240px,.82fr) minmax(0,1.8fr)); gap:clamp(22px,3vw,32px); }
  .footer-content.has-legal-status { grid-template-columns:var(--igf-footer-content-tablet-columns,repeat(2,minmax(0,1fr))); gap:28px 24px; }
...
@media(max-width:860px) {
  ...
  .footer-navigation,.footer-content.has-legal-status>.footer-navigation { display:grid; grid-template-columns:1fr; gap:0; order:2; border-top:1px solid var(--igf-footer-border,rgba(255,255,255,.12)); }
  .footer-link-group { border-bottom:1px solid var(--igf-footer-border,rgba(255,255,255,.12)); }
  .footer-link-group__heading { margin:0; }
  .footer-link-group__links { padding:2px 0 14px; }
  .footer-link-group__links>:deep(.managed-menu-tree) { grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px 18px; }
  .footer-link-group__links :deep(.managed-menu-tree[data-menu-depth="3"]) { grid-template-columns:1fr; }
...
@media(max-width:520px) {
  ...
  .footer-link-group__links>:deep(.managed-menu-tree) { grid-template-columns:1fr; }
}
</style>
