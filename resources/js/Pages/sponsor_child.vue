          ...
          <v-form ref="sponsorForm" v-model="isFormValid" class="igf-request-form" autocomplete="on" @submit.prevent="handleSubmit">
            <div class="igf-form-grid">
              <template v-for="field in formFields" :key="field.key">
                <label v-if="field.key === 'contribution_interval'" class="igf-native-field" for="sponsorship-interval"><span>{{ sponsorFieldLabel(field.key) }}</span><select id="sponsorship-interval" v-model="sponsorship.contributionInterval" :aria-label="sponsorFieldLabel(field.key)" required><option v-for="interval in contributionIntervals" :key="interval.value" :value="interval.value">{{ interval.label }}</option></select></label>
                <v-text-field
                  v-else
                  :model-value="sponsorFieldValue(field.key)"
                  :label="sponsorFieldLabel(field.key)"
                  :autocomplete="sponsorFieldAutocomplete(field.key)"
                  :type="sponsorFieldType(field.key)"
                  variant="outlined"
                  :min="field.key === 'number_of_children' ? 1 : undefined"
                  :max="field.key === 'number_of_children' ? 100 : undefined"
                  hide-details="auto"
                  :rules="sponsorFieldRules(field)"
                  :required="field.required"
                  @update:model-value="setSponsorFieldValue(field.key, $event)"
                />
              </template>
            </div>
...
const defaultFormFields = [
  { key: 'number_of_children', enabled: true, required: true },
  { key: 'contribution_interval', enabled: true, required: true },
...
const sponsorModelKeys = { number_of_children: 'numberOfChildren', contribution_interval: 'contributionInterval', name: 'name', email: 'email', phone: 'phone', address: 'address' };
const formFields = computed(() => {
  const managed = Array.isArray(settings.value.form_fields) ? settings.value.form_fields : defaultFormFields;
  return managed.filter(field => field && sponsorModelKeys[field.key] && field.enabled !== false);
});
...
function sponsorFieldValue(key) { return sponsorship.value[sponsorModelKeys[key]]; }
function setSponsorFieldValue(key, value) { sponsorship.value[sponsorModelKeys[key]] = value; }
function sponsorFieldLabel(key) {
  return {
    number_of_children: settings.value.children_field_label,
    contribution_interval: settings.value.interval_field_label,
    name: settings.value.name_field_label,
    email: settings.value.email_field_label,
    phone: settings.value.phone_field_label,
    address: settings.value.address_field_label,
  }[key] || key;
}
function sponsorFieldAutocomplete(key) { return { name: 'name', email: 'email', phone: 'tel', address: 'street-address' }[key]; }
function sponsorFieldType(key) { return { number_of_children: 'number', email: 'email', phone: 'tel' }[key] || 'text'; }
function sponsorFieldRules(field) {
  if (field.key === 'number_of_children') return childRules.value;
  if (field.key === 'email') return emailRules.value;
  return field.required ? [required(sponsorFieldLabel(field.key))] : [];
}
