...
<style>
    ...
    .hub-head{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;margin-bottom:28px}.hub-head h1{margin:0 0 7px;font:700 clamp(38px,4vw,58px)/1.05 'Literata',serif;letter-spacing:-.035em}.hub-head p{max-width:660px;margin:0;color:var(--muted);font-size:15px}.hub-create{display:inline-flex;align-items:center;gap:8px;padding:12px 18px;border:0;border-radius:9px;background:var(--orange);color:#fff;font-weight:800;text-decoration:none;box-shadow:0 5px 14px rgba(255,117,0,.18)}
    .hub-grid{display:grid;grid-template-columns:240px minmax(0,1fr);gap:24px}.hub-side{display:grid;align-content:start;gap:16px}.hub-card{border:1px solid var(--line);border-radius:11px;background:#fff;box-shadow:0 7px 24px rgba(25,28,29,.035)}.hub-card__body{padding:18px}.hub-card h2{margin:0 0 14px;font:600 18px 'Literata',serif}.hub-count{display:flex;justify-content:space-between;padding:7px 0;color:var(--muted);text-decoration:none}.hub-count strong{color:var(--primary)}.hub-count.is-active{color:var(--primary);font-weight:800}.hub-filter{display:grid;gap:10px}.hub-filter label{margin:2px 0 -4px;color:var(--muted);font-size:11px;font-weight:800;text-transform:uppercase}.hub-filter input,.hub-filter select{width:100%;padding:10px;border:1px solid #ddd8d2;border-radius:7px;background:#fff}.hub-filter__check{display:flex!important;align-items:center;gap:8px;margin:3px 0!important;font-size:12px!important;text-transform:none!important}.hub-filter__check input{width:auto}.hub-reset{color:var(--primary);font-size:12px;text-decoration:none}
    .hub-main{min-width:0}.hub-tools{display:flex;gap:10px;margin-bottom:12px}.hub-search{display:flex;flex:1}.hub-search input{width:100%;padding:11px 13px;border:1px solid #ddd8d2;border-right:0;border-radius:8px 0 0 8px}.hub-search button{padding:0 16px;border:1px solid #ddd8d2;border-radius:0 8px 8px 0;background:#fff}.hub-quick{display:flex;gap:7px}.hub-quick a{display:inline-flex;align-items:center;padding:9px 12px;border:1px solid var(--line);border-radius:8px;background:#fff;color:#504e4c;text-decoration:none;font-size:13px;font-weight:700}
...
<main class="content-hub">
    <header class="hub-head">
        ...
        @if($canCreatePage)<a class="hub-create" href="{{ route('page.create') }}"><span aria-hidden="true">+</span> Quick Create</a>@endif
    </header>
    ...
    <div class="hub-grid">
        <aside class="hub-side" aria-label="Content filters">
            ...
            <section class="hub-card"><div class="hub-card__body"><div style="display:flex;justify-content:space-between"><h2>Filters</h2><a class="hub-reset" href="{{ route('page.index') }}">Clear</a></div><form class="hub-filter" method="GET">
                ...
                <button class="hub-create" type="submit">Apply filters</button>
            </form></div></section>
        </aside>

        <section class="hub-main">
            <div class="hub-tools"><form class="hub-search" method="GET" action="{{ route('page.index') }}">@if($status)<input type="hidden" name="status" value="{{ $status }}">@endif @if($language)<input type="hidden" name="language" value="{{ $language }}">@endif @if($category)<input type="hidden" name="category" value="{{ $category }}">@endif @if($needsTranslation)<input type="hidden" name="needs_translation" value="1">@endif<input type="search" name="search" value="{{ $search }}" placeholder="Search content by title, URL, or category"><button type="submit" aria-label="Search">&#128269;</button></form></div>
            @if($hasQuickTools)
            ...
            <div class="hub-bulk">
                ...
                <div class="hub-bulk__actions" id="hub-bulk-actions" aria-label="Bulk actions">
                    @if($canBulkCopy)
            ...
            <div class="hub-list">
                @forelse($pages as $page)
                    ...
                    <article class="hub-row" id="page-{{ $page->id }}" data-page-uuid="{{ $page->uuid }}">
                        ...
                        <div class="hub-thumb">@if($page->thumbnail)<img src="{{ asset('storage/photos/1/page/' . $page->thumbnail) }}" alt="">@else<span aria-hidden="true">&#9635;</span>@endif</div>
                        ...
                        <div><span class="hub-badge hub-badge--{{ $state }}">{{ str_replace('_', ' ', $state) }}</span>@if((int)$page->translation_count < $languages->count())<span class="hub-translation">Needs translation</span>@endif @if($canViewSeo)<a class="hub-seo {{ $page->seo_admin_status==='Ready' ? 'hub-seo--ready' : ($page->seo_admin_status==='Hidden' ? 'hub-seo--hidden' : '') }}" href="{{ route('seo.content.edit', ['type'=>'page','id'=>$page->id,'locale'=>$page->language]) }}">SEO {{ $page->seo_admin_status }} · {{ $page->seo_admin_score }}%</a>@endif</div>
                        <div class="hub-updated"><span class="hub-meta">Updated<br>{{ $page->updated_at?->diffForHumans() }}</span></div>
                        <div class="hub-actions">
                            <a class="hub-action" href="{{ $previewUrl }}" target="_blank" rel="noopener" title="Preview" aria-label="Preview {{ $page->name }}">&#128065;</a>
                            @if($canEditRow)<a class="hub-action" href="{{ $editUrl }}" title="{{ $usesSponsorCustomizer ? 'Open Sponsor customizer' : 'Open visual editor' }}" aria-label="{{ $usesSponsorCustomizer ? 'Customize' : 'Edit' }} {{ $page->name }}">&#9998;</a>@endif
                            @if($canViewSeo)<a class="hub-action" href="{{ route('seo.content.edit', ['type'=>'page','id'=>$page->id,'locale'=>$page->language]) }}" title="Search &amp; sharing" aria-label="View SEO for {{ $page->name }}">&#128269;</a>@endif
                            @if($canDeletePage)<button class="hub-action trash" type="button" data-url="{{ route('page.destroy', $page->uuid) }}" data-id="{{ $page->uuid }}" title="Move to trash" aria-label="Trash {{ $page->name }}">&#128465;</button>@endif
                        </div>
                    </article>
...
const updateBulkState = () => {
    ...
    bulkActions?.classList.toggle('is-active', count > 0);
    if (selectAll) {
