...
class SeoSchemaTemplateService
{
    ...
    public function suggestedFor(string $kind, ?string $routeName = null): string
    {
        return match (true) {
            ...
            in_array($routeName, ['frontend.donate.index', 'frontend.donate.direct'], true) => 'donate',
            $kind === 'donation_cause' => 'donate',
            in_array($routeName, ['frontend.events', 'frontend.news', 'frontend.blog', 'frontend.project', 'frontend.gallery', 'frontend.annual_report.index'], true) => 'collection',
    ...
    public function detect(?array $schema): string
    {
        ...
        return match ($type) {
            ...
            'event' => 'event',
            'article', 'blogposting', 'newsarticle', 'report' => 'article',
            'donateaction' => 'donate',
            'webpage' => 'webpage',
            default => 'expert',
        };
