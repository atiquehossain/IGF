<template>
  <header ref="navRoot" class="site-nav">
    ...
    <nav id="mobile-navigation" ref="mobileNavigation" class="mobile-nav" :hidden="!drawer" :aria-label="header.mobileNavigationLabel" @keydown="handleMobileNavigationKeydown">
      ...
      <div v-if="utilityNavigation.length || showLanguage" class="mobile-nav__small-screen-extras">
        <section v-if="utilityNavigation.length" class="mobile-nav__extra-section mobile-nav__utility" aria-labelledby="mobile-utility-navigation-heading">
          <h2 id="mobile-utility-navigation-heading" class="mobile-nav__extra-heading">{{ header.utilityNavigationLabel }}</h2>
          <ManagedMenuTree
            :items="utilityNavigation"
            :max-depth="3"
            disclosure
            drawer-controls
            id-prefix="mobile-utility"
            :open-label-template="header.openSubmenuLabel"
            :close-label-template="header.closeSubmenuLabel"
            @navigate="closeMenus"
          />
        </section>

        <section v-if="showLanguage" class="mobile-nav__extra-section" aria-labelledby="mobile-language-navigation-heading">
          <h2 id="mobile-language-navigation-heading" class="mobile-nav__extra-heading">{{ header.languageSwitcherLabel }}</h2>
          <div class="mobile-nav__languages" role="group" aria-labelledby="mobile-language-navigation-heading">
            <a
...
const utilityNavigation = computed(() => (
  Array.isArray(inertiaPage.props.appUtilityMenus)
    ? inertiaPage.props.appUtilityMenus.filter(item => item && typeof item === 'object' && String(item.name || '').trim())
    : []
));
...
const header = computed(() => ({
  ...
  utilityNavigationLabel: inertiaPage.props.siteSettings?.header?.utility_navigation_label || 'Utility navigation',
  mobileNavigationLabel: inertiaPage.props.siteSettings?.header?.mobile_navigation_label || 'Mobile navigation',
  languageSwitcherLabel: inertiaPage.props.siteSettings?.header?.language_switcher_accessible_label
    || (currentLocale.value === 'bn' ? 'ভাষা' : 'Language'),
...
<style scoped>
.site-nav { position:relative; z-index:50; border-bottom:1px solid var(--igf-header-border,#e5e0dc); background:var(--igf-header-nav-bg,#fff); color:var(--igf-header-strong,#191c1d); font-family:var(--igf-font-body,'Hanken Grotesk',Arial,sans-serif); }
.site-nav__inner { display:flex; width:min(calc(100% - 40px),var(--igf-content-width,1240px)); height:var(--igf-header-nav-height,80px); align-items:center; justify-content:space-between; gap:24px; margin:0 auto; }
.site-brand { display:inline-flex; width:calc(var(--igf-brand-font-size,20px) * 5); min-width:calc(var(--igf-brand-font-size,20px) * 5); height:var(--igf-header-nav-height,80px); align-items:center; justify-content:center; text-decoration:none; }
.site-brand__logo { display:block; width:100%; height:var(--igf-header-nav-height,80px); object-fit:contain; }
...
.site-nav :deep(.desktop-nav__item>a),.site-nav :deep(.desktop-nav__trigger),.site-nav :deep(.desktop-nav__parent-row>a),.sign-in { position:relative; border:0; background:transparent; color:var(--igf-header-text,#56575b); font:800 12px/1.2 var(--igf-font-body,'Hanken Grotesk',Arial,sans-serif); letter-spacing:.025em; text-decoration:none; text-transform:uppercase; white-space:nowrap; cursor:pointer; }
...
.site-nav :deep(.desktop-nav__item>a::after),.site-nav :deep(.desktop-nav__trigger::after),.site-nav :deep(.desktop-nav__item>.desktop-nav__parent-row>a::after) { position:absolute; right:0; bottom:0; left:0; height:2px; transform:scaleX(0); background:var(--igf-primary,#ff7500); content:''; transition:.18s; }
.site-nav :deep(.desktop-nav__item>a:hover),.site-nav :deep(.desktop-nav__item>a.active),.site-nav :deep(.desktop-nav__trigger:hover),.site-nav :deep(.desktop-nav__trigger.active),.site-nav :deep(.desktop-nav__item>.desktop-nav__parent-row>a:hover),.site-nav :deep(.desktop-nav__item>.desktop-nav__parent-row>a.active),.sign-in:hover { color:var(--igf-accent,#9c4500); }
...
.site-nav :deep(.desktop-nav__toggle) { display:grid; width:44px; min-width:44px; height:44px; place-content:center; border:0; border-radius:var(--igf-radius-sm,7px); background:transparent; color:var(--igf-header-text,#56575b); cursor:pointer; }
.site-nav :deep(.desktop-nav__toggle:hover) { background:var(--igf-header-hover-bg,#f6f3f0); color:var(--igf-accent,#9c4500); }
.site-nav :deep(.desktop-nav__dropdown) { position:absolute; z-index:60; top:calc(100% - 12px); left:-16px; display:grid; min-width:250px; padding:10px; border:1px solid var(--igf-header-border,#e5e0dc); border-radius:var(--igf-radius-md,10px); background:var(--igf-header-nav-bg,#fff); box-shadow:var(--igf-shadow-floating,0 14px 34px rgba(25,28,29,.16)); }
...
.site-nav :deep(.desktop-nav__child),.site-nav :deep(.desktop-nav__branch) { display:grid; width:100%; min-height:44px; gap:3px; align-items:center; padding:9px 12px; border:0; border-radius:var(--igf-radius-sm,7px); background:var(--igf-header-nav-bg,#fff); color:var(--igf-header-text,#45464a); font:inherit; text-align:left; text-decoration:none; text-transform:none; cursor:pointer; }
...
.site-nav :deep(.desktop-nav__parent-row.is-nested>a) { display:grid; min-height:44px; gap:3px; align-content:center; padding:9px 12px; border-radius:var(--igf-radius-sm,7px); color:var(--igf-header-text,#45464a); text-decoration:none; text-transform:none; }
...
.site-nav :deep(.desktop-nav__entry:is(.is-active,.is-open)>.desktop-nav__branch),.site-nav :deep(.desktop-nav__child:hover),.site-nav :deep(.desktop-nav__child.active),.site-nav :deep(.desktop-nav__parent-row.is-nested>a:hover),.site-nav :deep(.desktop-nav__parent-row.is-nested>a.active),.site-nav :deep(.desktop-nav__branch:hover) { background:var(--igf-header-active-bg,color-mix(in srgb,var(--igf-primary,#ff7500) 10%,var(--igf-surface,#f8f9fa))); color:var(--igf-accent,#9c4500); }
.site-nav :deep(.desktop-nav__entry:is(.is-active,.is-open)>:is(.desktop-nav__branch,.desktop-nav__parent-row) small),.site-nav :deep(.desktop-nav__child:hover small),.site-nav :deep(.desktop-nav__child.active small) { color:var(--igf-header-active-text,var(--igf-accent,#9c4500)); }
.site-nav__actions { display:flex; align-items:center; gap:14px; }
.nav-icon { display:grid; width:44px; height:44px; place-content:center; border-radius:50%; color:var(--igf-header-text,#56575b); }
.nav-icon:hover { background:var(--igf-header-utility-bg,#f0f1f2); color:var(--igf-accent,#9c4500); }
.donate-button { display:inline-flex; min-height:44px; align-items:center; justify-content:center; gap:8px; padding:0 20px; border-radius:var(--igf-button-radius,999px); background:var(--igf-primary,#ff7500); color:var(--igf-on-primary,#000); font-size:12px; font-weight:800; letter-spacing:.04em; text-decoration:none; text-transform:uppercase; box-shadow:var(--igf-shadow-control,0 5px 14px color-mix(in srgb,var(--igf-primary,#ff7500) 18%,transparent)); }
.donate-button:hover { background:var(--igf-accent,#9c4500); color:var(--igf-on-accent,#fff); }
.sponsor-button { display:inline-flex; min-height:44px; align-items:center; justify-content:center; padding:0 15px; border:1px solid var(--igf-accent,#9c4500); border-radius:var(--igf-button-radius,999px); color:var(--igf-accent,#9c4500); font-size:11px; font-weight:800; letter-spacing:.03em; text-decoration:none; text-transform:uppercase; white-space:nowrap; }
.sponsor-button:hover { border-color:var(--igf-accent,#9c4500); background:color-mix(in srgb,var(--igf-primary,#ff7500) 9%,var(--igf-header-nav-bg,#fff)); color:var(--igf-accent,#9c4500); }
.menu-button { display:none; width:44px; height:44px; place-content:center; border:0; border-radius:var(--igf-radius-md,9px); background:var(--igf-header-utility-bg,#f0f1f2); color:var(--igf-header-strong,#191c1d); font-size:19px; cursor:pointer; }
.mobile-action-bar { display:none; padding-bottom:10px; background:var(--igf-header-nav-bg,#fff); }
...
.mobile-nav { position:absolute; z-index:60; top:100%; right:0; left:0; display:grid; max-height:min(75vh,calc(100vh - var(--igf-header-nav-height,80px))); max-height:min(75dvh,calc(100dvh - var(--igf-header-nav-height,80px))); overflow:auto; overscroll-behavior:contain; padding:16px 20px 24px; border-bottom:1px solid var(--igf-header-border,#ddd); background:var(--igf-header-nav-bg,#fff); box-shadow:var(--igf-shadow-floating,0 16px 30px rgba(0,0,0,.12)); }
.mobile-nav[hidden] { display:none; }
.site-nav :deep(.mobile-nav__group>a),.site-nav :deep(.mobile-nav__parent),.site-nav :deep(.mobile-nav__parent-row>a) { display:flex; width:100%; min-height:48px; align-items:center; justify-content:space-between; padding:11px 8px; border:0; border-bottom:1px solid var(--igf-header-border,#eee); background:var(--igf-header-nav-bg,#fff); color:var(--igf-header-strong,#292a2d); font:750 15px/1.3 var(--igf-font-body,'Hanken Grotesk',Arial,sans-serif); text-align:left; text-decoration:none; cursor:pointer; }
.site-nav :deep(.mobile-nav__entry :is(.mobile-nav__parent,.mobile-nav__child,.mobile-nav__parent-row>a)) { border-bottom:0; }
.site-nav :deep(.mobile-nav__parent i),.site-nav :deep(.mobile-nav__toggle i) { color:var(--igf-accent,#9c4500); font-size:11px; transition:transform .18s ease; }
...
.site-nav :deep(.mobile-nav__toggle) { display:grid; min-width:48px; min-height:48px; place-content:center; border:0; border-bottom:1px solid var(--igf-header-border,#eee); background:var(--igf-header-nav-bg,#fff); cursor:pointer; }
.site-nav :deep(.mobile-nav__submenu) { padding:5px 0 8px; background:var(--igf-header-utility-bg,#faf8f6); }
...
.site-nav :deep(.mobile-nav__group.is-active>:is(.mobile-nav__link,.mobile-nav__parent)),.site-nav :deep(.mobile-nav__group.is-active>.mobile-nav__parent-row>a),.site-nav :deep(.mobile-nav__entry.is-active>.mobile-nav__parent),.site-nav :deep(.mobile-nav__child:hover),.site-nav :deep(.mobile-nav__child.active) { border-left-color:var(--igf-primary,#ff7500); background:var(--igf-header-active-bg,color-mix(in srgb,var(--igf-primary,#ff7500) 10%,var(--igf-surface,#f8f9fa))); color:var(--igf-header-active-text,var(--igf-accent,#9c4500)); }
.site-nav :deep(.mobile-nav__group.is-active>:is(.mobile-nav__link,.mobile-nav__parent)),.site-nav :deep(.mobile-nav__group.is-active>.mobile-nav__parent-row>a) { box-shadow:inset 3px 0 var(--igf-primary,#ff7500); }
...
.mobile-nav__small-screen-extras { display:none; gap:16px; padding-top:16px; border-top:1px solid var(--igf-header-border,#e5e0dc); margin-top:16px; }
.mobile-nav__extra-section { display:grid; gap:6px; }
.mobile-nav__extra-heading { margin:0; color:var(--igf-header-text,#56575b); font:800 11px/1.3 var(--igf-font-body,'Hanken Grotesk',Arial,sans-serif); letter-spacing:.08em; text-transform:uppercase; }
.site-nav :deep(.mobile-nav__utility .managed-menu-tree) { display:grid; width:100%; }
.site-nav :deep(.mobile-nav__utility .managed-menu-tree[hidden]) { display:none; }
.site-nav :deep(.mobile-nav__utility .managed-menu-tree[data-menu-depth="2"]),.site-nav :deep(.mobile-nav__utility .managed-menu-tree[data-menu-depth="3"]) { padding:5px 0 6px 16px; border-left:2px solid var(--igf-header-border,#e5e0dc); background:var(--igf-header-utility-bg,#faf8f6); }
.site-nav :deep(.mobile-nav__utility .managed-menu-tree__parent-row) { display:grid; grid-template-columns:minmax(0,1fr) 48px; }
.site-nav :deep(.mobile-nav__utility :is(.managed-menu-tree__link,.managed-menu-tree__label,.managed-menu-tree__disclosure)) { display:flex; width:100%; min-height:44px; align-items:center; justify-content:space-between; gap:10px; padding:10px 8px; border:0; border-bottom:1px solid var(--igf-header-border,#e5e0dc); background:transparent; color:var(--igf-header-strong,#292a2d); font:700 14px/1.35 var(--igf-font-body,'Hanken Grotesk',Arial,sans-serif); text-align:left; text-decoration:none; }
.site-nav :deep(.mobile-nav__utility .managed-menu-tree__disclosure) { cursor:pointer; }
.site-nav :deep(.mobile-nav__utility .managed-menu-tree__toggle) { display:grid; min-width:48px; min-height:44px; place-content:center; border:0; border-bottom:1px solid var(--igf-header-border,#e5e0dc); background:transparent; color:var(--igf-accent,#9c4500); cursor:pointer; }
.site-nav :deep(.mobile-nav__utility :is(.managed-menu-tree__toggle,.managed-menu-tree__disclosure) i) { font-size:10px; transition:transform .18s ease; }
.site-nav :deep(.mobile-nav__utility :is(.managed-menu-tree__toggle,.managed-menu-tree__disclosure) i.is-rotated) { transform:rotate(180deg); }
.site-nav :deep(.mobile-nav__utility :is(.managed-menu-tree__link,.managed-menu-tree__disclosure):hover) { background:var(--igf-header-active-bg,color-mix(in srgb,var(--igf-primary,#ff7500) 10%,var(--igf-surface,#f8f9fa))); color:var(--igf-header-active-text,var(--igf-accent,#9c4500)); }
.mobile-nav__languages { display:flex; flex-wrap:wrap; gap:8px; }
.mobile-nav__languages a { display:grid; min-width:64px; min-height:44px; place-content:center; padding:8px 14px; border:1px solid var(--igf-header-border,#e5e0dc); border-radius:var(--igf-radius-sm,7px); color:var(--igf-header-strong,#292a2d); font-weight:800; text-decoration:none; }
.mobile-nav__languages a[aria-current="page"] { border-color:var(--igf-primary,#ff7500); background:var(--igf-primary,#ff7500); color:var(--igf-on-primary,#000); }
.site-nav :deep(a:focus-visible),.site-nav :deep(button:focus-visible) { outline:3px solid var(--igf-primary,#ff7500); outline-offset:3px; }
@media(max-width:1460px) { .site-nav__actions>.sponsor-button.site-nav__inline-action { display:none; } }
@media(max-width:1180px) { .desktop-nav,.sign-in { display:none; } .menu-button { display:grid; } .mobile-nav { display:grid; max-height:min(75vh,calc(100vh - var(--igf-header-nav-height,80px) - var(--igf-header-utility-height,38px))); max-height:min(75dvh,calc(100dvh - var(--igf-header-nav-height,80px) - var(--igf-header-utility-height,38px))); } }
@media(min-width:1181px) { .mobile-nav { display:none; } }
@media(max-width:767px) { .mobile-nav { max-height:min(75vh,calc(100vh - var(--igf-header-nav-height,80px))); max-height:min(75dvh,calc(100dvh - var(--igf-header-nav-height,80px))); } .mobile-nav__small-screen-extras { display:grid; } }
@media(max-width:720px) { .site-nav__actions>.site-nav__inline-action { display:none; } .mobile-action-bar { display:block; } .mobile-nav { max-height:min(75vh,calc(100vh - var(--igf-header-nav-height,80px) - 54px)); max-height:min(75dvh,calc(100dvh - var(--igf-header-nav-height,80px) - 54px)); } }
@media(max-width:560px) { .site-nav__inner { width:calc(100% - 28px); height:var(--igf-header-nav-height-mobile,70px); } .site-brand { width:82px; min-width:82px; height:var(--igf-header-nav-height-mobile,70px); } .site-brand__logo { width:82px; height:calc(var(--igf-header-nav-height-mobile,70px) - 6px); } .mobile-action-bar__inner { width:calc(100% - 28px); } .donate-button { font-size:11px; } .site-nav__actions { gap:8px; } .mobile-nav { max-height:min(75vh,calc(100vh - var(--igf-header-nav-height-mobile,70px) - 54px)); max-height:min(75dvh,calc(100dvh - var(--igf-header-nav-height-mobile,70px) - 54px)); } }
