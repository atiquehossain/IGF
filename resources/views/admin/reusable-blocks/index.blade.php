...
@php
    ...
    $canEditReusableBlocks = $permissions->allows($admin, 'reusable-blocks.edit');
...
<main class="igf-reuse">
    ...
    <div class="igf-table-wrap table-responsive" role="region" aria-label="Scrollable reusable section records" tabindex="0"><table class="igf-table">
        <thead><tr><th>Name</th><th>Type</th><th>Language</th><th>Instances / pages</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
        @forelse($blocks as $block)
            <tr><td><strong>{{ $block->name }}</strong></td><td>{{ $blockTypes[$block->type] ?? $block->type }}</td><td>{{ $block->locale === '*' ? 'All languages' : strtoupper($block->locale) }}</td><td>{{ $block->instances_count }}</td><td><span class="igf-badge">{{ $block->is_enabled ? 'Active' : 'Disabled' }}</span></td><td><div class="igf-actions">
                ...
                @else
                    <a class="igf-btn" href="{{ route('reusable-blocks.show', $block) }}"><i class="fa fa-folder-open" aria-hidden="true"></i> Open</a>
                    <a class="igf-btn" href="{{ route('reusable-blocks.preview', ['reusableBlock' => $block, 'locale' => $block->locale === '*' ? $locale : $block->locale]) }}" target="_blank" rel="noopener"><i class="fa fa-eye" aria-hidden="true"></i> Preview</a>
                    @if($canEditReusableBlocks)<a class="igf-btn igf-btn--primary" href="{{ route('reusable-blocks.edit', $block) }}"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>@endif
                    @if($canDeleteReusableBlocks)
