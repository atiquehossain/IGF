<template>
  <Layout>
    <div class="igf-opportunities">
      ...
      <section class="igf-opportunities__content" :aria-labelledby="listingTitleId">
        <div class="igf-shell">
          <header class="igf-opportunities__heading">
            <h2 :id="listingTitleId">{{ copy.listing_title }}</h2>
          ...
          <div v-if="items.length" class="igf-opportunities__grid">
            <OpportunityCard
              ...
              :listing="listing"
              :href="listing.public_url || route('frontend.jobs.show', listing.slug)"
              kind="job"
              :copy="cardCopy"
            />
          </div>
          <div v-else class="igf-opportunities__empty" role="status">
            ...
            <p>{{ copy.empty_message }}</p>
          </div>
          <v-pagination
            v-if="properties.total_page > 1"
            :model-value="properties.page || 1"
            :length="properties.total_page"
            class="igf-opportunities__pagination"
            :aria-label="copy.pagination_label"
...
const title = computed(() => page.props.title || 'Careers')
const items = computed(() => data.value.items || [])
const properties = computed(() => page.props.properties || {})
const listingTitleId = 'career-listing-title'
const copy = computed(() => ({
