...
@section('content')
@php
    $availableLocaleIds = $locales->pluck('id')->map(fn ($id) => (string) $id)->all();
    $requestedLanguage = (string) request()->query('language', '');
    $defaultLanguage = in_array($requestedLanguage, $availableLocaleIds, true)
        ? $requestedLanguage
        : (string) ($locales->first()->id ?? 'en');
    $selectedLanguage = (string) old('language', $defaultLanguage);
    $requestedCategorySlug = trim((string) request()->query('category_slug', ''));
    $requestedTagSlug = trim((string) request()->query('tag_slug', ''));
    $preselectedCategory = $categorylist->first(fn ($category) =>
        (string) $category->language === $selectedLanguage
        && (string) $category->slug === $requestedCategorySlug
    );
    $preselectedTag = $tags->first(fn ($tag) => (string) $tag->slug === $requestedTagSlug);
    $selectedCategoryId = (string) old('category_id', $preselectedCategory?->id ?? '');
    $selectedTagIds = array_map('strval', old('tags', $preselectedTag ? [$preselectedTag->id] : []));
    $creationKind = in_array(request()->query('kind'), ['program', 'project', 'blog'], true)
        ? (string) request()->query('kind')
        : 'page';
    $creationLabel = $creationKind === 'blog' ? 'blog post' : $creationKind;
@endphp
<style>
...
<main class="draft-wizard">
    ...
    <header class="draft-head">
        ...
        <h1>Create one {{ $creationLabel }} draft</h1>
    ...
    <form class="draft-form" action="{{ route('page.store') }}" method="POST">
        ...
        <div class="draft-card">
            <section class="draft-section" aria-labelledby="draft-basics-heading">
                <h2 id="draft-basics-heading">{{ $creationKind === 'blog' ? 'Blog post basics' : 'Page basics' }}</h2>
                    ...
                    <div class="draft-field">
                        ...
                        <select id="draft-category" name="category_id" data-locale-select>
                            ...
                            @foreach($categorylist as $category)
                                <option value="{{ $category->id }}" data-locale-option="{{ $category->language }}" @selected($selectedCategoryId === (string) $category->id)>{{ $category->name }}</option>
                            @endforeach
                        </select>
                            ...
                            <div class="draft-tag-list">
                                @foreach($tags as $tag)
                                    <label class="draft-tag">
                                        <input type="checkbox" name="tags[]" value="{{ $tag->id }}" @checked(in_array((string) $tag->id, $selectedTagIds, true))>
                                        <span>{{ $tag->name }}</span>
                                    </label>
                                @endforeach
                            </div>
